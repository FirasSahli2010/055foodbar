        ...
        <div class="details_size">
            ...
            <div class="form-check">
                ...
                <label class="form-check-label" for="size-{{ $productSize->id }}">
                    {{ $productSize->name }} <span>+ {{ $settings->currency_icon }} {{ $productSize->price }}</span>
                </label>
            </div>
            @endforeach
        </div>
        ...
        <div class="details_extra_item">
            ...
            <div class="form-check">
                ...
                <label class="form-check-label" for="option-{{ $productOption->id }}">
                    {{ $productOption->name }} <span>+{{ $settings->currency_icon }} {{$productOption->price}}</span>
                </label>
            </div>
            @endforeach
        </div>
    ...
    <div class="details_quentity">
        ...
        <div class="quentity_btn_area d-flex flex-wrapa align-items-center">
            <div class="quentity_btn">
                ...
                <input type="text" id="quantity" name="qty" placeholder="1" value="1" readonly>
                <button class="btn btn-success increment"><i class="fal fa-plus"></i></button>
            </div>
            @if ($product->offer_price > 0)
            <h3 id="total_price"> {{ $settings->currency_icon }} {{$product->offer_price}}</h3>
            @else
            <h3 id="total_price">{{ $settings->currency_icon }} {{$product->price}}</h3>
            @endif
        </div>
    </div>
...
<script>
    $(document).ready(function(){
        ...
        function updateTotalPrice() {
            ...
            $('#total_price').text("{{ $settings->currency_icon}}" + totalPrice);
